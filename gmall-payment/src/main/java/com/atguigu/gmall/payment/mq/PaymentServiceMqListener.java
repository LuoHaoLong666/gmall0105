...
public class PaymentServiceMqListener {
    ...
    public void consumePaymentCheckResult(MapMessage mapMessage) throws JMSException {
            ...
            // 调用paymentService的支付宝检查接口,属于支付服务中的操作，要在PaymentService进行编写。
            ...
            if(resultMap!=null&&!resultMap.isEmpty()){
                ...
                // 根据查询的支付状态结果，判断是否进行下一次的延迟任务还支付成功更新数据和后续任务
                if(StringUtils.isNotBlank(trade_status)&&trade_status.equals("TRADE_SUCCESS")){
                    ...
                    paymentInfo.setAlipayTradeNo((String)resultMap.get("trade_no"));  // 支付宝的支付凭证号
                    ...
                    System.out.println("已经支付成功，调用支付服务修改支付信息和发送支付成功的队列");
                    return;
                }
            }
            ...
            if(count>0){
                // 继续发送延迟检查任务，计算延迟时间等
                System.out.println("没有支付成功，检查剩余次数为"+count+"调用发送延迟检查任务");
                count--;
