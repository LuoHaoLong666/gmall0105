...
@Service  // @Service不仅是Spring中的bean，同时也是一个提供dubbo协议的一个RPC服务。
...
//        }
    ...
    @Override
    public void addUserToken(String token, String memberId) {  // 添加用户Token到redis缓存中
        Jedis jedis = redisUtil.getJedis();

        jedis.setex("user:"+memberId+":token",60*60*2,token);  // 带过期时间的redis
